...
import { readRunState, RUN_OUTPUT_ROOT } from "../runner/run-output.js";
...
const metrics = [
  ...
  { label: "Static off-origin references", value: offOrigin.length, scope: "static scan of scope for off-origin url()/@import; runtime requests are measured by the published-preview smoke test" },
  ...
  { label: "Contrast failures", value: contrast.violations.length, scope: "contrast gate: designed pairs below threshold (text 4.5:1 + non-text 3:1); undesigned-pairing membership is enforced at test time by the Playwright runner" },
